...
import { dayKey, emptyState, makeSession, parseBackup, submitSession } from './core';
...
export default function App(){
 ...
 async function start(){
  ...
  const next={...state,session:{day:today,deck,queue:makeSession(state,words,deck,today)}};
 ...
 async function choose(d:Deck){if(!state||d===deck)return;if(await commit({...state,settings:{...state.settings,deck:d},session:null}))setSearch('');}
 ...
 const cardContent=(w:Word)=><><div className="word-heading"><span className="word-pos">{w.pos}</span><h3>{w.meaning}</h3></div><div className="example"><div className="eyebrow">放进句子里记住</div><p lang="en">{w.example}</p><p className="translation">{w.translation}</p><button className="text-button" onClick={()=>speak(w.example)}><Volume2 size={16}/>朗读例句</button></div></>;
 return <div className="app-shell">
  ...
  {page==='library'&&<section className="library-page"><div className="page-intro"><span className="eyebrow">WORD COLLECTION</span><h1>选一个目标，<br className="mobile-only"/>慢慢积累。</h1><p>精选版，非完整考试词表。跨词库的相同单词共享进度。</p></div><div className="deck-groups">{([['四六级',['cet4','cet6']],['雅思托福',['ielts','toefl']]] as const).map(([name,list])=><div key={name}><h2>{name}</h2><div className="deck-tabs">{list.map(d=><button key={d} disabled={busy} className={deck===d?'selected':''} onClick={()=>choose(d)}>{labels[d]}<small>150 词</small>{deck===d&&<Check size={16}/>}</button>)}</div></div>)}</div><label className="search-box"><Search size={19}/><input placeholder="搜索单词或中文释义" value={search} onChange={e=>setSearch(e.target.value)}/>{search&&<button aria-label="清除搜索" onClick={()=>setSearch('')}><X size={17}/></button>}</label><div className="section-heading"><h2>{titles[deck]}<span className="muted"> · 精选</span></h2><span className="muted">已学 {selected.filter(w=>state.progress[w.id]).length} / 150</span></div><div className="word-list">{selected.filter(w=>`${w.word} ${w.meaning}`.toLowerCase().includes(search.trim().toLowerCase())).map(w=><button key={w.id} onClick={()=>setDetail(w)}><span><strong lang="en">{w.word}</strong><small>{w.pos} {w.meaning}</small></span><span className={`word-status ${state.progress[w.id]?'seen':''}`}>{state.progress[w.id]?'已学':'未学'}<ArrowUpRight size={16}/></span></button>)}</div>{!selected.some(w=>`${w.word} ${w.meaning}`.toLowerCase().includes(search.trim().toLowerCase()))&&<div className="empty-state">没有找到这个词，试试其他英文或中文关键词。</div>}</section>}
  ...
  {page==='me'&&<section className="me-page"><div className="page-intro"><span className="eyebrow">A LITTLE, EVERY DAY</span><h1>看见自己的积累。</h1><p>不用和别人比，每天比昨天多记住一点。</p></div><div className="personal-stats"><div><Flame size={24}/><strong>{studiedDays}</strong><span>累计学习天数</span></div><div><BookOpen size={24}/><strong data-testid="learned-count">{learned}</strong><span>已学独立词数</span></div></div><section className="settings-card"><h2>每天学多少新词？</h2><p>到期复习不占新词名额，四个词库共用每日目标。</p><div className="goal-options">{[5,10,15,20].map(n=><button disabled={busy} key={n} className={state.settings.dailyGoal===n?'selected':''} onClick={()=>commit({...state,settings:{...state.settings,dailyGoal:n},session:null})}>{n}<small>词 / 天</small></button>)}</div></section><section className="settings-card"><h2>把积累，好好保存</h2><p>进度只保存在当前设备。清除网站数据会丢失记录，建议定期导出备份。安装到主屏幕后，请确认进度是否一致；需要时可导入备份。</p><div className="backup-actions"><button className="secondary" onClick={exportBackup}><Download size={18}/>导出备份</button><label className="secondary upload"><Upload size={18}/>导入备份<input aria-label="导入备份文件" type="file" accept=".json,application/json" disabled={busy} onChange={e=>{void importBackup(e.target.files?.[0]);e.target.value='';}}/></label></div></section><section className="settings-card install-card"><div><h2>让词记住进你的主屏幕</h2><p>像 App 一样打开，随时学一点。</p></div><button className="icon-button" aria-label="查看安装说明" onClick={()=>setInstall(true)}><ArrowUpRight size={22}/></button></section><p className="about-copy">每日词记 1.0 · 精选版，非完整考试词表<br/>300 个独立词条 · 4 个词库各 150 词<br/>原创释义与例句 · 系统英语语音 · 无需账号</p></section>}
  </main>
