...
import { own, addWord, allWords, setNote, dayKey, emptyState, makeSession, parseBackup, submitSession } from './core';
...
export default function App(){
 ...
 useEffect(()=>{setNoteDraft(detail&&state?own(state.notes,detail.id)??'':'');},[detail]);
 useEffect(()=>{
 ...
 const due=selected.filter(w=>{const p=own(state.progress,w.id);return p&&p.due<=today;}).length;
 ...
 const cardContent=(w:Word)=><><div className="word-heading"><span className="word-pos">{w.pos}</span><h3>{w.meaning}</h3></div>{w.example&&<div className="example"><div className="eyebrow">放进句子里记住</div><p lang="en">{w.example}</p><p className="translation">{w.translation}</p><button className="text-button" onClick={()=>speak(w.example)}><Volume2 size={16}/>朗读例句</button></div>}{own(state.notes,w.id)&&<div className="saved-note"><strong>我的备注</strong><p>{own(state.notes,w.id)}</p></div>}</>;
